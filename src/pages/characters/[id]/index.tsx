...
import { getOneOrMoreEpisodes } from "@/lib/episode/episode";
import { Episode } from "@/lib/episode/types";
import {
  Card,
  CardMedia,
  CardContent,
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const character = await getCharacter((context.params?.id as string) || "");
  const episodes = await getOneOrMoreEpisodes(
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Aditional details of ${character.name}`} />
      ...
      <main>
        <Toolbar>
          ...
          <Breadcrumbs sx={{ overflowX: 'auto' }} aria-label="breadcrumb">
        ...
        <Grid minHeight="calc(100vh - 200px)">
          <Card sx={{ flexDirection: 'column', justifyContent: 'center', display: "flex", boxShadow: "none" }}>
            <CardMedia
              component="img"
              sx={{ width: 120, alignSelf: 'center', height: 120, borderRadius: 50 }}
            ...
            <Box>
              ...
              <Box sx={{ display: "none", alignItems: "center", pl: 1, pb: 1 }}>
                <h2>Content</h2>
              </Box>
            </Box>
          </Card>
                  ...
                  {episodes?.map((episode) => {
                    return (
                      <ListItem key={episode.id}>
                        <ListItemAvatar>
                          <Avatar sx={{ bgColor: "background.blue-5" }}>
                            <LiveTvIcon />
                          </Avatar>
                        </ListItemAvatar>
                        <ListItemText
